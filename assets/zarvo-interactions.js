/* ══════════════════════════════════════════════════════════════
   ZARVO — Interaction Layer (zarvo-interactions.js)
...
(function () {
  ...
  window.matchMedia('(prefers-reduced-motion: reduce)').addEventListener('change', function (e) {
    if (e.matches) {
      ...
      var host = document.getElementById('zarvo-cursor-host');
  ...
  function insertDOM() {
    /* Page curtain */
    if (!document.getElementById('zarvo-curtain')) {
      var curtain = document.createElement('div');
      curtain.id = 'zarvo-curtain';
    ...
    /* Custom cursor — lives inside a Shadow DOM so zoom extensions
       ...
       The HOST element (#zarvo-cursor-host) IS found by the extension
    ...
    if (!document.getElementById('zarvo-cursor-host') && !TOUCH) {
      var host = document.createElement('div');
      host.id  = 'zarvo-cursor-host';
    ...
    if (document.querySelector('.hero-title') && !document.getElementById('zarvo-success')) {
      var success = document.createElement('div');
      success.id = 'zarvo-success';
      success.innerHTML =
        '<div class="zarvo-success-rule"></div>' +
        '<div class="zarvo-success-copy">' +
          '<span class="zarvo-success-eyebrow">Submission received</span>' +
          '<span class="zarvo-success-headline">We\'ll be<br>in touch.</span>' +
        '</div>';
  ...
  function markAndObserve(container) {
    ...
    ctx.querySelectorAll(
      ...
      if (!el.classList.contains('zarvo-reveal-label')) {
        el.classList.add('zarvo-reveal-label');
    ...
    ctx.querySelectorAll('.q-card, .client-card').forEach(function (card) {
      ...
      fields.forEach(function (el, i) {
        if (!el.classList.contains('zarvo-reveal')) {
          el.classList.add('zarvo-reveal');
    ...
    ctx.querySelectorAll('.section-head').forEach(function (el) {
      ...
      if (!el.classList.contains('zarvo-reveal-rule')) {
        el.classList.add('zarvo-reveal-rule');
        if (!REDUCED) obs.observe(el); else el.classList.add('is-revealed');
      }
    });
    ...
    if (footer && !footer.classList.contains('zarvo-reveal')) {
      footer.classList.add('zarvo-reveal');
  ...
  function initWizardObserver() {
    ...
    pages.forEach(function (page) {
      if (page.classList.contains('active')) {
        ...
        if (!REDUCED) {
          setTimeout(function () {
            page.querySelectorAll('.zarvo-reveal, .zarvo-reveal-label').forEach(function (el) {
              el.classList.add('is-revealed');
            });
        ...
        if (m.type === 'attributes' && m.attributeName === 'class') {
          ...
          if (page.classList.contains('active') && !seen.has(page.id)) {
            ...
            var allReveal = Array.from(page.querySelectorAll(
              '.zarvo-reveal, .zarvo-reveal-label, .zarvo-reveal-rule'
            ));
  ...
  function initInputFeedback() {
    ...
    document.addEventListener('focusout', function (e) {
      ...
      if (!val) {
        ...
        if (label) {
          label.classList.remove('zarvo-shake');
          /* Reflow so the animation re-triggers if already running */
          void label.offsetWidth;
          label.classList.add('zarvo-shake');
  ...
  /* ══════════════════════════════════════════════════════
     7. SUBMIT BUTTON
     Wraps existing text in .zarvo-btn-inner so CSS can
  ...
  function initSubmitButton() {
    ...
    /* Wrap content in .zarvo-btn-inner if not already done */
    function wrapContent() {
      if (!btn.querySelector('.zarvo-btn-inner') &&
          !btn.querySelector('.zarvo-loading-dashes')) {
        btn.innerHTML = '<span class="zarvo-btn-inner">' + btn.innerHTML + '</span>';
      }
    }
    wrapContent();
      ...
      mutations.forEach(function (m) {
        if (m.attributeName === 'data-loading') {
          if (btn.getAttribute('data-loading') === 'true') {
            ...
            btn.innerHTML =
              '<span class="zarvo-loading-dashes">' +
              '<span class="d">\u2014</span>' +
              '<span class="d">\u2014</span>' +
              '<span class="d">\u2014</span>' +
              '</span>';
        ...
        if (m.type === 'childList') {
          var hasLoader = !!btn.querySelector('.zarvo-loading-dashes');
          var isLoading = btn.getAttribute('data-loading') === 'true';
          if (!hasLoader && !isLoading && !btn.querySelector('.zarvo-btn-inner')) {
            wrapContent();
          }
        }
      });
  ...
  function initSuccessOverlay() {
    ...
    var successEl    = document.getElementById('zarvo-success');
