...
-- Removes question-order prefixes and converts to snake_case names.
--
-- Safe behavior:
...
-- ── Phase 2: Rename hyphenated columns to snake_case ──────────────────────
-- Handles databases that already ran Phase 1 and have the intermediate
-- hyphenated names (e.g. "business-description" → business_description).
DO $$
DECLARE
  rec record;
BEGIN
  FOR rec IN
    ...
    FROM (
      VALUES
        ...
        ('customer-desire',        'customer_desire'),
        ('brand-personality',      'brand_personality'),
        ('launch-context',         'launch_context'),
        ('brands-admired',         'brands_admired'),
        ('colors-to-avoid',        'colors_to_avoid'),
        ('aesthetic-description',  'aesthetic_description'),
        ('existing-assets',        'existing_assets'),
        ('inspiration-refs',       'inspiration_refs'),
        ('decision-maker',         'decision_maker'),
        ('decision-maker-other',   'decision_maker_other'),
  ...
  LOOP
    IF EXISTS (
      ...
      IF EXISTS (
        ...
        EXECUTE format(
          'UPDATE public.submissions SET %I = COALESCE(%I, %I) WHERE %I IS NULL AND %I IS NOT NULL',
          rec.new_col, rec.new_col, rec.old_col, rec.new_col, rec.old_col
        );
        EXECUTE format('ALTER TABLE public.submissions DROP COLUMN %I', rec.old_col);
      ELSE
